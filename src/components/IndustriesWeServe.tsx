  ...
  return (
    <section id="industries" className="min-h-screen flex flex-col justify-center py-12 lg:py-16 bg-brand-dark border-t border-white/5 relative overflow-clip">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-14 lg:gap-y-16 gap-x-4 md:gap-x-6">
          {industries.map((industry, idx) => (
            <motion.div
              ...
              className="group relative bg-[#121212] border border-white/5 hover:border-brand-primary/30 p-6 pt-20 md:pt-24 rounded-2xl flex flex-col items-center text-center transition-colors duration-500 hover:bg-[#1A1A1A] hover:shadow-2xl hover:shadow-brand-primary/10 mt-8 h-full"
            >
              {/* Floating Image overlapping the card */}
              <div 
                className="absolute -top-16 md:-top-20 left-1/2 -translate-x-1/2 w-36 h-36 md:w-44 md:h-44 pointer-events-none drop-shadow-2xl group-hover:-translate-y-3 transition-transform duration-500"
              >
                <Image 
              ...
              <h3 
                className="font-heading text-lg md:text-xl font-medium tracking-tight text-brand-light uppercase mt-2 md:mt-4"
              >
                {industry.name}
              </h3>
            </motion.div>
